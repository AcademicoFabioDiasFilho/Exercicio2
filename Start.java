...
final class Start {
    ...
    private static void comecar() {
        ...
        */
        ...
        if (!escreverMensagem(String.format("Grupo\nMaior altura: %.2fm\nMenor altura: %.2fm", maiorAltura, menorAltura))) {
            return;
        }

        if (!Double.isNaN(mediaDeAlturaDosMachos)) {
            if (!escreverMensagem(String.format("Média de altura dos homens: %.2fm", mediaDeAlturaDosMachos))) {
                return;
            }
        } else if (!escreverMensagem("Média de altura dos homens: Indefinida.")) {
            return;
        }

        if (!escreverMensagem(String.format("Número de mulheres: %d", numeroDeFemeas))) {
            System.exit(0);
        }
    }
